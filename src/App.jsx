...
export default function App() {
  ...
  useEffect(() => {
    if (!isCloudMode) {
      const demoOrders = [
        { id: '1', customerId: 'cust-1', customerName: '演示客户', phone: '13800000000', address: '本地模式', remark: '', deposit: 500, computerSn: 'DEMO-01', startDate: '2026-03-11', days: 30, monthlyRent: 300, paidRent: 300, renewMonths: 0, status: 'active', createdAt: Date.now(), logs: [{time: Date.now(), msg: '✨ 订单初始创建'}] }
      ];
  ...
  const handleAddCustomer = async () => {
    ...
    const newOrder = {
      customerId: cid, customerName: '新客户', phone: '', address: '', remark: '', deposit: 0,
  ...
  const handleAddDevice = async (customerId, customerName, phone, address, remark, deposit) => {
    ...
    const newOrder = {
      customerId, customerName, phone, address, remark: remark || '', deposit: Number(deposit) || 0,
      computerSn: '', startDate: new Date().toISOString().split('T')[0], days: 30, monthlyRent: 0, paidRent: 0, renewMonths: 0, status: 'active',
        ...
        <div className="p-4 md:p-6 bg-[#262930] flex flex-col gap-5 border-b border-[#333842]">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-5 border-t border-[#333842] pt-4 mt-1">
             ...
             <div className="flex flex-col sm:flex-row w-full md:w-auto items-center gap-3 shrink-0">
                <button onClick={() => onAddDevice(cid, data.name, data.phone, data.address, data.remark, data.deposit)} className="w-full sm:w-auto flex justify-center items-center gap-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 rounded-lg text-white font-bold transition shadow-lg">
                  <Plus size={18} /> 新增设备订单
                </button>
...
function EquipmentTab({ computers, orders, isCloudMode, user, db, appId, onPreviewImage, preferences }) {
  ...
  const roiPercent = totalRoi * 100;
  const roiRingPercent = Math.min(Math.max(roiPercent, 0), 100);
    ...
    <div className="pb-20">
      ...
      <div className="bg-[#22252b] rounded-xl p-6 mb-6 border border-gray-800 flex flex-col md:flex-row items-center gap-6 shadow-sm">
        <div
          ...
          style={{ background: `conic-gradient(${roiColor} ${roiRingPercent}%, #1F1F1F ${roiRingPercent}% 100%)` }}
          aria-label={`资产整体回本 ${roiPercent.toFixed(0)}%`}
        >
           <div className="absolute inset-[10px] rounded-full bg-[#22252b]"></div>
           <span className="relative z-10 font-bold text-white text-lg">{roiPercent.toFixed(0)}%</span>
        </div>
